...
void loop() {
  ...
  if(digitalRead(3)==HIGH && play_lectura==1){
    play_lectura = 0;
  ...
  if(digitalRead(4)==HIGH && play_lectura==1){
    play_lectura = 0;
  ...
  if(digitalRead(5)==HIGH && play_lectura==1){
    play_lectura = 0;

    // sizeof gives the number of bytes, each int value is composed of two bytes (16 bits)
    // there are two values per note (pitch and duration), so for each note there are four bytes
    int notes = sizeof(melody4) / sizeof(melody4[0]) / 2;
